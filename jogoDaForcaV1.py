...
def mostrarSituacaoDaRodada(qnt_erros, tema, palavra, palavra_escondida,
                            letras_digitadas):
    mostrarForca(qnt_erros)
...
def checarLetra(letra, letras_digitadas, palavra):
...
def adicionarLetraChutada(letra, letras_digitadas):
    letras_digitadas.append(letra)
    return letras_digitadas


def marcarAcerto(letra, palavra, palavra_escondida, letras_digitadas):
    ...
    letras_digitadas = adicionarLetraChutada(letra, letras_digitadas)
    # Retorna palavra_escondida atualizada e letras_digitadas atualizada
    return palavra_escondida, letras_digitadas


def marcarErro(erros, letras_digitadas, letra):
    # Verifica se o usurio apena digitou "Enter" sem ter escrito nada
    if len(letra) != 0:
        erros += 1
        # Verifica se o conteudo é uma palavra ou um número.
        # OBS: Defini que nesse jogo o usuario só podera digitar letra/letra
        # E que nas palavra nao podera conter números.
        ...
        tipo_de_erro = 3
    mostrarTipoErro(tipo_de_erro, letra)
    return erros, letras_digitadas


def mostrarTipoErro(tipo, letra):
    tipos_erros = ['Por favor, chute apenas letras!\n'
                   'Você está perdendo pontos.',
                   f'Você já chutou a letra "{letra}", cuidado. Errou.',
                   f'Não possui a letra "{letra}". Errou.',
                   'Por favor, experimente chutar alguma letra.\n'
                   'Nenhum ponto será descontado.'
                   ]
    mensagem(tipos_erros[tipo], 'vermelho')
...
def main():
    ...
    while continuar:
        ...
        while erros < 6 and palavra_escondida != gabarito:
            mostrarSituacaoDaRodada(erros, tema, palavra, palavra_escondida,
                                    letras_digitadas)
            letra = tratarString(input('Digite uma letra: '))
            if checarLetra(letra, letras_digitadas, palavra) is True:
                palavra_escondida, letras_digitadas = marcarAcerto(
                    letra, palavra, palavra_escondida, letras_digitadas)
            else:
                erros, letras_digitadas = marcarErro(erros, letras_digitadas,
                                                     letra)
            print('-' * 43)
